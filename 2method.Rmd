...
**Note to Farhad: The function below is referring to biomass and bins even though this hasn't been previously mentioned.  This makes me think that Section 2 should be describing the biomass aspect of this.  This could go in part before Section 2.1 and in part during Section 2.1 if you agree.  One approach could be to create a new section before the current Section 2.1 that describes the form of the data (e.g. bins vs particles, biomass, etc). Question for you: Do you think that `generate_smooth_gauss_mix()` should also generate biomasses?**

**Note to Farhad: For a user of the package (who will not be reading this bookdown but rather using `?kernel_em`), it'd be good to add a bit of explanation below where I have written "[Add here]" (see line 12).  This can be a few lines explaining some things like the use of `y` versus `biomass`.  Also, it'd be good to describe the format of `initial_fit`.**

**Note to Farhad: You'll see that I've made the code a bit more succinct in terms of the initialization of `initial_fit`.  I got rid of the initialization code chunk since it was too short to justify its own code chunk.  However, the initialization section is so long and complex, I would suggest moving this section to after the E-step and M-step sections.  I think as a reader I first want to understand the main algorithm and then be told about initialization.**
...
**Note to Farhad: In general it's best to avoid repeating the same lines of code in different places.  I'm noticing multiple places where you need to calculate responsibilities.  I would suggest defining a function called something like `calculate_responsibilities()` that takes a `mu`, `Sigma`, and `pi` as input.**
...
**Note to Farhad: In terms of double checking, it would be good to write out more details here.  In particular, for such a calculation, we'd like to know what the prior is (stating hyperparameter values not just "Dirichlet") and then what the random variable is whose parameters are described by the prior.  And then what the posterior is.  And then finally the below equation would be the posterior mean.  Here's my attempt to do this (would be good for you to verify and then do the same for the other parameters beyond $\pi_t$):**

At time $t$ we imagine estimating the number of points in each class $k$, which is

$$
n_t\hat{\pi}_t|\pi_t\sim\text{Multinomial}(n_t,\pi_{t})
$$
where $\hat{\pi}_t,\pi_t$ are $K$-dimensional probability vectors.  Our prior takes the previous 

$$\pi_{t}\sim \text{Dirichlet}(n_{t-1}\hat{\pi}_{t-1}).$$
(Here we are conditioning on everything that happened before time $t$, but omitting this in the notation.)

Thus, the posterior is

$$
\pi_t|n_t\hat\pi_t\sim \text{Dirichlet}(n_t\hat\pi_t+n_{t-1}\hat{\pi}_{t-1})
$$

and the posterior mean is thus

$$
\mathbb E[\pi_t|n_t\hat\pi_t]=\frac{n_t\hat\pi_t+n_{t-1}\hat{\pi}_{t-1}}{\sum_{k=1}^K[n_t\hat\pi_{tk}+n_{t-1}\hat{\pi}_{t-1,k}]}=\frac{n_t\hat\pi_t+n_{t-1}\hat{\pi}_{t-1}}{n_t+n_{t-1}}.
$$
In other words, we estimate the vector $\pi_t$ to be the convex combination $\alpha_t\hat\pi_t+(1-\alpha_t)\hat\pi_{t-1}$, where $\alpha_t=n_t/(n_t+n_{t-1})$.

**Note to Farhad: This is the end of my suggested description of the $pi$ part of the Bayesian method.  I didn't tackle the biomass aspect of this, so I'll leave it to you to figure out how this can be adapted to that case.**
...
For $\mu_{tk}$, we assume a prior that's a multivariate normal distribution with mean $\mu_{t-1,k}$ and covariance matrix $\frac{1}{n_{t-1,k}} \hat\Sigma_{t-1,k}$. We then get the following posterior mean:

$$
\hat{\mu}_{tk} = \frac{\left(\sum_{i=1}^{n_{t-1}} \tilde\gamma_{i,t-1,k}\right) \mu_{k,t-1} + \sum_{i=1}^{n_t} \tilde\gamma_{itk} Y_{it}}
{\left(\sum_{i=1}^{n_{t-1}} \tilde\gamma_{i,t-1,k}\right) + \left(\sum_{i=1}^{n_t} \tilde\gamma_{itk}\right)}
$$
We then estimate $\hat\Sigma_{tk}$ using the posterior mean again, assuming a prior that's an inverse-Wishart with mean $\hat\Sigma_{t-1,k}$. Our estimate is:
...
A bit more thought is required regarding what multiple iterations (default number of iterations is 1) of this Bayesian EM-algorithm would do (on the second iteration and above, we use $\mu_{kt}$ from the previous iteration in the place of $\mu_{t-1,k}$, and similarly for $\Sigma_{tk}$ and $\pi_{tk}$). We also add the option to introduce Laplace smoothing for calculating the responsibilities, to prevent $pi_{tk}$ collapsing to 0 for some clusters (the default Laplace smoothing constant is 0). Here is the function that implements this approach:

**Note to Farhad: I agree with your comment below that more explanation about Laplace smoothing is needed.  Unless you don't think we need it anymore, in which case it can be removed.**

**Note to Farhad: You should describe in the text what is going on with `matrixStats::rowLogSumExps()`.  Also, this part would probably be packaged in the function `calculate_responsibilities()` that I've suggested you create.**
...
fit$resp[[50]][1:4, ]
```
We see that the algorithm is pretty certain that the first bin at time point 50 belongs to cluster 2, the second to cluster 2 also, the third to cluster 1, and the 4th to cluster 3. We might not always have this certainty for points that are somewhat "midway" between different cluster centers.  For example:
